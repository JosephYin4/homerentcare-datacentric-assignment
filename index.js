...
const dbname = "homerentcare"; // make sure this matches your database in mongo
...
async function main() {
    ...
    // Get all property details (with search functionality)
    app.get('/propertydetail', async (req, res) => {
        try {
            const { _id, nameofProperty, homeownerFullname, address, postalcode, numberofBedrooms, numberofBathrooms, carparkLots, amenities } = req.query;
            let query = {};
           ...
           // }
            ...
            if (_id) {
               query['_id'] = { $regex: _id, $options: 'i' };
            }

            if (nameofProperty) {
                query['nameofProperty'] = { $regex: nameofProperty, $options: 'i' };
            }

            if (homeownerFullname) {
                query['homeownerFullname'] = { $regex: homeownerFullname, $options: 'i' };
            }
            ...
            if (numberofBedrooms) {
                query['numberofBedrooms'] = { $regex: numberofBedrooms, $options: 'i' };
            }

            if (numberofBathrooms) {
                query['numberofBathrooms'] = { $regex: numberofBathrooms, $options: 'i' };
            }

            if (carparkLots) {
                query['carparkLots'] = { $regex: carparkLots, $options: 'i' };
            }
            ...
            const propertydetail = await db.collection('propertydetail').find(query).project({
                '_id': 1,
                'nameofProperty': 1,
                'homeownerFullname': 1,
                'address': 1,
            ...
            console.error('Error searching property details:', error);
            res.status(500).json({ error: 'Internal server error' });
        }
    });

        // Get all user details (with search functionality)
        app.get('/userdetail', async (req, res) => {
            try {
                const { _id, type, fullname, contactnumber, emailaddress, propertyid } = req.query;
                let query = {};
               ...
               // }
                ...
                if (propertyid) {
                    query['propertyid'] = { $regex: propertyid, $options: 'i' };
                }
    
                const userdetail = await db.collection('userdetail').find(query).project({
                    '_id': 1,
                    'type': 1,
                    'fullname': 1,
                    'contactnumber': 1,
